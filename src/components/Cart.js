...
export default function Cart({ cart, setCart, books, setBooks, mode = 'page' }) {
  ...
  const updateQuantity = (bookId, newQuantity) => {
    ...
    if (quantityChange > 0 && currentBook.b_count < quantityChange) {
      alert(`אין מספיק במלאי! זמין: ${currentBook.b_count}`);
      return;
    }
            ...
            <div key={item.id} className="cart-item">
              ...
              <div className="quantity-controls">
                ...
                <button 
                  ...
                  disabled={books.find(b => b.id === item.id)?.b_count === 0}
                  title={books.find(b => b.id === item.id)?.b_count === 0 ? 'אין במלאי' : ''}
                >
                  +
                </button>
              </div>
